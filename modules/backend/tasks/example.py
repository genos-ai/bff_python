...
Task function definitions for Taskiq background processing.
Functions are plain async coroutines; they get registered with the broker
via register_tasks() when Redis is available.
...
async def send_notification(
    ...
    logger.info(
        ...
        extra={"user_id": user_id, "channel": channel, "message_length": len(message)},
    )

    # TODO: Implement actual delivery logic per channel
    ...
    logger.info("Notification sent", extra={"user_id": user_id, "channel": channel})
    return result
...
async def process_data(
    ...
    """
    logger.info("Processing data", extra={"operation": operation, "data_keys": list(data.keys())})
    ...
    logger.info("Data processing completed", extra={"operation": operation, "duration_ms": duration_ms})
    return result
...
async def cleanup_expired_records(
    ...
    Returns:
        Cleanup statistics
    """
    logger.info("Starting cleanup", extra={"table_name": table_name, "older_than_days": older_than_days})
    ...
    logger.info("Cleanup completed", extra={"table_name": table_name, "deleted_count": deleted_count})
    return result
...
async def generate_report(
    ...
    """
    logger.info("Starting report generation", extra={"report_type": report_type, "user_id": user_id})
    ...
    logger.info("Report generation completed", extra={"report_id": report_id, "user_id": user_id})
    return result
